...
export function CategoryScreen() {
  ...
  return <AppScreen padded={false}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}><View style={styles.tabs}><Pressable style={styles.tab} onPress={() => router.back()}><AppText variant="subtitle" color={theme.colors.subtle}>Home</AppText></Pressable><View style={styles.active}><AppText variant="subtitle">Category</AppText></View></View><Pressable style={styles.feature} onPress={() => router.push(`/search?category=${feature.id}` as Href)}><View><AppText variant="subtitle">{feature.name}</AppText><AppText variant="caption">{feature.count} Product</AppText></View><Image source={feature.image} style={styles.featureImage} contentFit="cover" /></Pressable><View style={styles.list}>{categories.slice(1).map((category) => <Pressable key={category.id} style={styles.category} onPress={() => router.push(`/search?category=${category.id}` as Href)}><Image source={category.image} style={StyleSheet.absoluteFill} contentFit="cover" /><View style={styles.copy}><AppText variant="subtitle">{category.name}</AppText><AppText variant="caption">{category.count} Product</AppText></View></Pressable>)}</View></ScrollView></AppScreen>;
}
