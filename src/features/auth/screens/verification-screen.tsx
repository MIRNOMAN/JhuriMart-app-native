        ...
        <View style={styles.otp}>
          {digits.map((digit, index) => (
            <TextInput
              ...
              style={[styles.cell, index === 3 && styles.activeCell]}
            />
          ))}
        </View>
        <AppButton
          label="Submit"
          onPress={() => router.push("/register-success" as Href)}
        />
        <AppText variant="caption" color={theme.colors.muted}>
          Didn&apos;t receive the code?{" "}
          <AppText variant="caption" color={theme.colors.violet}>
            Resend
          </AppText>
        </AppText>
      </View>
...
const styles = StyleSheet.create({
  content: { flex: 1, alignItems: "center", paddingTop: 50, gap: 14 },
  iconRing: {
    width: 76,
    height: 76,
    borderRadius: 38,
  ...
  otp: { flexDirection: "row", gap: 9, marginVertical: 18 },
  cell: {
    width: 48,
    height: 48,
    borderRadius: 10,
